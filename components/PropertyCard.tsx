...
export default function PropertyCard({ item, onPress }: Props) {
  const { t } = useLanguage();
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={() => onPress?.(item.id)} style={styles.card}>
      <Image
        source={{ uri: item.imageUrl || 'https://picsum.photos/seed/prop/400/240' }}
        style={styles.image}
      />
      <View style={styles.content}>
        <View style={styles.headerRow}>
          ...
          <View style={[styles.badge, item.verified ? styles.badgeVerified : styles.badgeUnverified]}>
            <Text style={styles.badgeText}>{item.verified ? t('verified') : t('notVerified')}</Text>
          </View>
        </View>
